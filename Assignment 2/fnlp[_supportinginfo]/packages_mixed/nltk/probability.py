...
#
# $Id: probability.py 8392 2009-11-02 01:36:01Z nmadnani $
...
class ProbabilisticMixIn(object):
    ...
    def __init__(self, **kwargs):
        ...
        @kwparam cost: negative log probability
        @type cost: C{float}
        """
        if ((('prob' in kwargs)+
            ('logprob' in kwargs)+
            ('cost' in kwargs))>1):
                raise TypeError('Must specify at most one of prob, logprob or cost')
        if ('prob' in kwargs):
                ProbabilisticMixIn.set_prob(self, kwargs['prob'])
        elif 'logprob' in kwargs:
            ProbabilisticMixIn.set_logprob(self, kwargs['logprob'])
        elif 'cost' in kwargs:
            ProbabilisticMixIn.set_logprob(self, -kwargs['cost'])
        else:
    ...
    def set_cost(self, cost):
        """
        Set the cost associated with this object to
        C{cost}.  I.e., set the probability associated with this
        object to C{2**(-cost)}.
    ...
    def cost(self):
        """
        @return: C{-log(p,2)}, where C{p} is the probability associated
        with this object.
